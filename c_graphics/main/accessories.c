...
void writeConfigFile( const char *path, bool enableLog ) {
    ...
    if (initErrors == 0) {
        ...
        res = f_unlink( path );
		LV_LOG_USER("Error deleting configuration file: %s", path);
...
uint8_t caseInsensitiveStrstr(const char *haystack, const char *needle) {
    ...
    char *haystackLower = heap_caps_malloc(strlen(haystack) + 1, MALLOC_CAP_INTERNAL);	// This wants to be fast so use internal RAM!
