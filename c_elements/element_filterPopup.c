  ...
  if(obj == gui.element.filterPopup.mBoxApplyFilterButton || obj == gui.element.filterPopup.mBoxResetFilterButton){
      if(code == LV_EVENT_CLICKED) {    
        if(obj == gui.element.filterPopup.mBoxApplyFilterButton){
          ...
          if(resetPressed == 1){
            LV_LOG_USER("resetPressed %d", resetPressed); 
            lv_obj_set_style_text_color(gui.page.processes.iconFilterLabel, lv_color_hex(BLACK), LV_PART_MAIN);
  ...
  if(obj == gui.element.filterPopup.mBoxOnlyPreferredSwitch){
    ...
    if(code == LV_EVENT_VALUE_CHANGED) {
      if(gui.element.filterPopup.filterName == NULL && gui.element.filterPopup.isColorFilter == false && gui.element.filterPopup.isBnWFilter == false && gui.element.filterPopup.preferredOnly == false)
          ...
          resetPressed = 0;  /* User selected a new filter after reset — cancel the reset */
    }
}
