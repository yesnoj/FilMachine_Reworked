 ...
 */
typedef struct {
    const nv3041_lcd_init_cmd_t *init_cmds; /*!< Initialization commands array, set to NULL for default. */
    uint16_t init_cmds_size;                /*!< Number of commands in above array */
} nv3041_vendor_config_t;

/**
 * @brief Create LCD panel for model NV3041
 *
 ...
 *          - ESP_OK                on success
 */
esp_err_t esp_lcd_new_panel_nv3041(const esp_lcd_panel_io_handle_t io,
 ...
 */
#define NV3041_PANEL_BUS_SPI_CONFIG(sclk, mosi, max_trans_sz)  \
    {                                                           \
        .sclk_io_num = sclk,                                    \
        .mosi_io_num = mosi,                                    \
        .miso_io_num = -1,                                      \
 ...
 */
#define NV3041_PANEL_IO_SPI_CONFIG(cs, dc, callback, callback_ctx) \
    {                                                               \
        .cs_gpio_num = cs,                                          \
        .dc_gpio_num = dc,                                          \
        .spi_mode = 0,                                              \
