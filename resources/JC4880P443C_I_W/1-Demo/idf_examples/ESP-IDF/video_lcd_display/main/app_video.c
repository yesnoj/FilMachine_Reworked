...
static inline esp_err_t video_receive_video_frame(int video_fd)
{
    ...
    // ESP_LOGI(TAG,"video_receive_video_frame");
...
esp_err_t app_video_stream_task_start(int video_fd, int core_id, void *user_data)
{
    ...
    BaseType_t result = xTaskCreatePinnedToCore(video_stream_task, "video stream task", VIDEO_TASK_STACK_SIZE, &video_fd, 4, &app_camera_video.video_stream_task_handle, core_id);
