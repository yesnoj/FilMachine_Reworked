...
//            (lv_obj_t *)lv_obj_get_child(cont, *active_id);  // Not sure why this is here!
            uint8_t i = 0;
	...
	if (act_cb == gui.page.settings.filmRotationSpeedSlider) {
	    if (code == LV_EVENT_VALUE_CHANGED) {
	        ...
	        // Aggiungi o sottrai 10 al valore corrente  THIS IS NOT REQUIRED :-)
/*	        if (new_value > current_value) {
	        ...
	        if (remainder != 0) {
	            if (remainder > 5) {		// Round up
	                ...
	                new_value -= remainder;// (new_value / 10) * 10;
	            }
	        }
...
/*            if(new_value > current_value) {
	        ...
	        // Arrotonda new_value agli step di 20
	        int remainder = new_value % 20;
	        if (remainder != 0) {
	            if (remainder > 10) {		// Round up
	                new_value = ((new_value / 20) + 1) * 20;
	            } else {
	                new_value -= remainder;// (new_value / 10) * 10;
	            }
	        }
...
/*            if(new_value > current_value) {
            ...
            // Arrotonda new_value agli step di 10
	        int remainder = new_value % 50;
	        if (remainder != 0) {
	            if (remainder > 25) {		// Round up
	                new_value = ((new_value / 50) + 1) * 50;
	            } else {
	                new_value -= remainder;// (new_value / 10) * 10;
	            }
	        }
...
/*            if(new_value > current_value) {
	        ...
	        // Arrotonda new_value agli step di 10
	        int remainder = new_value % 30;
	        if (remainder != 0) {
	            if (remainder > 15) {		// Round up
	                new_value = ((new_value / 30) + 1) * 30;
	            } else {
	                new_value -= remainder;// (new_value / 10) * 10;
	            }
	        }
