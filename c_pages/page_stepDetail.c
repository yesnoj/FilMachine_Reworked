...
//  lv_obj_t * mboxParent = (lv_obj_t *)lv_obj_get_parent(mboxCont);
 ...
 if(code == LV_EVENT_CLICKED){
    if(obj == gui.tempStepNode->step.stepDetails->stepSaveButton){
      ...
      strncpy(gui.tempStepNode->step.stepDetails->stepNameString, lv_textarea_get_text(gui.tempStepNode->step.stepDetails->stepDetailNamelTextArea), MAX_PROC_NAME_LEN);
      gui.tempStepNode->step.stepDetails->stepNameString[MAX_PROC_NAME_LEN] = '\0';
  ...
  if(code == LV_EVENT_REFRESH){
    if(obj == gui.tempStepNode->step.stepDetails->stepSaveButton){
      if(gui.tempStepNode->step.stepDetails->isEditMode == false){ //means step is still not created
          if(strlen(lv_textarea_get_text(gui.tempStepNode->step.stepDetails->stepDetailNamelTextArea)) > 0 && (strtol(lv_textarea_get_text(gui.tempStepNode->step.stepDetails->stepDetailSecTextArea), NULL, 10) > 0 || strtol(lv_textarea_get_text(gui.tempStepNode->step.stepDetails->stepDetailMinTextArea), NULL, 10) > 0 )) {
