...
//        lv_obj_delete(mboxCont);
    ...
    if(data == gui.tempProcessNode->process.processDetails->processSaveButton && gui.tempProcessNode->process.processDetails->stepElementsList.size > 0){
          ...
          strncpy( gui.tempProcessNode->process.processDetails->processNameString, lv_textarea_get_text(gui.tempProcessNode->process.processDetails->processDetailNameTextArea), MAX_PROC_NAME_LEN);
          gui.tempProcessNode->process.processDetails->processNameString[MAX_PROC_NAME_LEN] = '\0';
...
if(existingProcess != NULL) {
  ...
  LV_LOG_USER("Process address 0x%p, with n:%"PRIu16" steps",gui.tempProcessNode, gui.tempProcessNode->process.processDetails->stepElementsList.size); 
            ...
            //lv_obj_set_style_border_opa(gui.tempProcessNode->process.processDetails->processInfoContainer, LV_OPA_TRANSP, 0);
                  ...
                  lv_obj_set_style_border_opa(gui.tempProcessNode->process.processDetails->processTempContainer, LV_OPA_TRANSP, 0);   
                          ...
                          if(gui.page.settings.settingsParams.tempUnit == CELSIUS_TEMP){
                              ...
                              sprintf(formatted_string, "%"PRIi32"", gui.tempProcessNode->process.processDetails->temp);
                              ...
                              sprintf(formatted_string, "%"PRIi32"", convertCelsiusoToFahrenheit(gui.tempProcessNode->process.processDetails->temp));
                              lv_textarea_set_text(gui.tempProcessNode->process.processDetails->processTempTextArea, formatted_string);
                          }  
...
//                          sprintf(formatted_string, "%dm%ds", gui.tempProcessNode->process.processDetails->timeMins, gui.tempProcessNode->process.processDetails->timeSecs);
                          lv_label_set_text_fmt(gui.tempProcessNode->process.processDetails->processTotalTimeValue, "%"PRIu32"m%"PRIu8"s", 
